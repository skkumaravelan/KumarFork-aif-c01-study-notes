...
- **CNN (Convolutional Neural Networks)**: 
  - **Algorithms**: LeNet, AlexNet, VGGNet, ResNet, Inception
  - **Use Cases**: Image classification, object detection, computer vision tasks
- **RNN (Recurrent Neural Networks)**: 
  - **Algorithms**: LSTM (Long Short-Term Memory), GRU (Gated Recurrent Unit), Vanilla RNN
  - **Use Cases**: Sequential data processing, time series analysis, natural language processing
- **GAN (Generative Adversarial Networks)**: 
  - **Algorithms**: DCGAN, StyleGAN, CycleGAN, Progressive GAN
...
- **Feature Extraction**: Use pre-trained model features, freeze weights, train new classifier
- **Fine-tuning**: Adapt pre-trained model weights to new task with small learning rate
- **Domain Adaptation**: Transfer knowledge between related domains

#### Semi-Supervised Learning
- **Self-training**: Model trained on labeled data predicts unlabeled data iteratively
- **Co-training**: Multiple models trained on different feature sets
- **Graph-based**: Uses relationships between data points

#### Self-Supervised Learning
- **Contrastive Learning**: Learn representations by contrasting similar/dissimilar examples
- **Masked Language Modeling**: Predict masked tokens (used in BERT)
- **Autoregressive**: Predict next token in sequence (used in GPT)
...
|-----------|------|----------|-----------|------------|
| **k-Means** | Partitional | Market segmentation | Fast, simple | Requires k specification |
| **Hierarchical** | Agglomerative/Divisive | Taxonomy creation | No k needed | Computationally expensive |
| **DBSCAN** | Density-based | Anomaly detection | Handles noise, arbitrary shapes | Parameter sensitive |
| **Gaussian Mixture** | Model-based | Soft clustering | Probabilistic assignments | Assumes Gaussian distributions |

#### Dimensionality Reduction Techniques
| Technique | Type | Purpose | Best For |
|-----------|------|---------|----------|
| **PCA** | Linear | Feature reduction | High-dimensional data |
| **t-SNE** | Non-linear | Visualization | Cluster visualization |
| **UMAP** | Non-linear | Embedding | Preserving global structure |
| **LDA** | Linear | Supervised reduction | Classification tasks |

#### Association Rule Learning
- **Apriori Algorithm**: Finds frequent itemsets using candidate generation
- **FP-Growth**: Tree-based approach for frequent pattern mining
- **Eclat**: Vertical data format based frequent itemset mining

#### Probability Density Estimation
- **Kernel Density Estimation (KDE)**: Non-parametric density estimation
- **Gaussian Mixture Models**: Parametric approach using mixture of Gaussians
- **Histogram-based**: Simple binning approach for density estimation
...
| **Feature Extraction** | Create new features from existing data | PCA, polynomial features, domain knowledge | SciKit-learn, AWS Glue |
...
|--------|---------|-------------|----------------|
| **Accuracy** | (TP+TN)/(TP+TN+FP+FN) | Balanced datasets | Overall correctness |
| **Precision** | TP/(TP+FP) | Cost of false positives high | Quality of positive predictions |
| **Recall (Sensitivity)** | TP/(TP+FN) | Cost of false negatives high | Coverage of actual positives |
| **F1-Score** | 2×(Precision×Recall)/(Precision+Recall) | Balance precision and recall | Harmonic mean of precision/recall |
| **Specificity** | TN/(TN+FP) | True negative rate important | Quality of negative predictions |

#### Performance Metrics
| Metric | Description | Use Case | Target |
|--------|-------------|----------|--------|
| **Average Response Time** | Mean time for model to return prediction | Real-time applications | <100ms for interactive |
| **Data Throughput** | Requests processed per second | Batch processing | Maximize for cost efficiency |
| **P95/P99 Latency** | 95th/99th percentile response time | SLA compliance | <500ms for most applications |

#### Advanced Evaluation Metrics
| Metric | Purpose | Use Case | Range |
|--------|---------|----------|-------|
| **ROUGE Score** | Text summarization quality | Summarization tasks | 0-1 (higher better) |
| **BLEU Score** | Translation quality | Machine translation | 0-1 (higher better) |
| **BERTScore** | Semantic similarity using embeddings | Text generation | -1 to 1 (higher better) |
